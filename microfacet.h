/// Este arquivo foi modificado por mim para adicionar o método lambda,
/// definido no código fornecido por Heitz em https://eheitzresearch.wordpress.com/240-2/.
...
public:
    ...
    /**
     * \brief Função lambda de Smith. Foi adaptada do código fornecido por Heitz.
     *
     * \param v
     *     Uma direção arbitrária.
     */
    Float lambda(const Vector3f &v) const {
        if (dr::all(v.z() > 0.9999f)) return 0.f;
        if (dr::all(v.z() < -0.9999f)) return -1.f;

        Float result = 0.f;

        const Float invSinTheta2 = 1.0f / (1.0f - dr::square(v.z()));
        const Float cosPhi2 = dr::square(v.x()) * invSinTheta2;
        const Float sinPhi2 = dr::square(v.y()) * invSinTheta2;
        const Float alpha = dr::sqrt(dr::square(m_alpha_u) * cosPhi2 + dr::square(m_alpha_v) * sinPhi2);

        const Float theta = dr::acos(v.z());
        const Float a = dr::rcp(dr::tan(theta) * alpha);

        if (m_type == MicrofacetType::Beckmann) {
            // Distribuição de Beckmann
            result = 0.5f * (dr::erf(a) - 1.f) +
                        dr::rcp(2.f * a * dr::sqrt(dr::Pi<Float>)) * dr::exp(-dr::square(a));
        } else {
            // Distribuição GGX / Trowbridge-Reitz
            result = 0.5f * (-1.f + ((dr::all(a > 0)) ? 1.f : -1.f) *
                                        dr::sqrt(1.f + dr::rcp((dr::square(a)))));
        }
